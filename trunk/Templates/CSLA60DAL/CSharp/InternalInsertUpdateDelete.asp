...
[InsertChild]
    ...
    %>private void Child_Insert([Inject] I<%= Info.ObjectName %>Dal dal<% if (parentType.Length > 0) { %>, <%= parentType %> parent<% } %>)
        {
            <%
...
if (Info.GenerateDataPortalUpdate)
{
    ...
    if (parentType.Length > 0 && !Info.ParentInsertOnly)
    {
        foreach (ValueProperty parentProp in Info.GetParentValueProperties())
        {
            ...
            if (usesDTO)
            {
                strUpdateDto += Environment.NewLine + new string(' ', 16) + "dto.Parent_" + FormatPascal(parentProp.Name) + " = parent." + parentProp.Name +";";
            }
            else
            {
                if (!updateIsFirst)
...
[UpdateChild]
    ...
    %>private void Child_Update([Inject] I<%= Info.ObjectName %>Dal dal<% if (parentType.Length > 0 && !Info.ParentInsertOnly) { %>, <%= parentType %> parent<% } %>)
        {
            <%
...
[DeleteSelfChild]
    ...
    else if (Info.TransactionType == TransactionType.TransactionScope)
    {
        ...
        %>private void Child_DeleteSelf([Inject] I<%= Info.ObjectName %>Dal dal<% if (parentType.Length > 0 && !Info.ParentInsertOnly) { %>, <%= parentType %> parent<% } %>)
        {
                <%
    if (TemplateHelper.UseSimpleAuditTrail(Info))
    {
