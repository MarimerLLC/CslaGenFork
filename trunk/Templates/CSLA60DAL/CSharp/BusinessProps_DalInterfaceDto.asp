...
// Child properties
int childCount = 0;
foreach (var prop in Info.GetCollectionChildProperties())
{
    childCount ++;    
    if (prop.LoadingScheme == LoadingScheme.ParentLoad)
    {
...
%>
        ...
        /// </summary>
        /// <value>The <%= CslaGenerator.Metadata.PropertyHelper.SplitOnCaps(prop.Name) %>.</value>
        public List<<%= childInfo.ItemType %>Dto> <%= FormatPascal(prop.Name) %> { get; set; } = new List<<%= childInfo.ItemType %>Dto>();
<%
    }
}
%>