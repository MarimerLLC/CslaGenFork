...
<%
    ...
    if (UseChildFactoryHelper)
    {
        %>
            ...
            var item = <%= Info.ItemType %>.<%= newMethodName %><%= crit.CreateOptions.FactorySuffix %>(<%= factoryParams %>, portal);
            <%
    }
    else
    {
        %>
            var portal = ApplicationContext.GetRequiredService<IChildDataPortal<<%= Info.ItemType %>>>();
            var item = portal.Create<%= crit.CreateOptions.RunLocal ? "Child" : "" %>(<%= factoryParams %>);
            <%
    }
    %>
            Add(item);
