...
<%
    ...
    if (authzInfo2.IsNotReadOnlyCollection() &&
		...
		(Info.InheritedType == null || !Info.InheritedType.FinalName.StartsWith("NetsoftBusinessBase")))
    {
        %>
        /// <summary>
        ...
        public static bool CanAddObject(ApplicationContext appContext)
        {
            return BusinessRules.HasPermission(appContext, Csla.Rules.AuthorizationActions.CreateObject, typeof(<%= Info.ObjectName %>));
        }

        <%
    }
	if (Info.InheritedType == null || 
		(!Info.InheritedType.FinalName.StartsWith("NetsoftBusinessBase") && 
			!Info.InheritedType.FinalName.StartsWith("NetsoftReadOnlyBase") &&
			!Info.InheritedType.FinalName.StartsWith("NetsoftReadOnlyListBase")))
	{
        %>
        /// <summary>
        ...
        {
            return BusinessRules.HasPermission(appContext, Csla.Rules.AuthorizationActions.GetObject, typeof(<%= Info.ObjectName %>));
        }
        <%
	}
    if (authzInfo2.IsNotReadOnlyCollection() &&
		...
		(Info.InheritedType == null || !Info.InheritedType.FinalName.StartsWith("NetsoftBusinessBase")))
    {
        ...
        public static bool CanEditObject(ApplicationContext appContext)
        {
            return BusinessRules.HasPermission(appContext, Csla.Rules.AuthorizationActions.EditObject, typeof(<%= Info.ObjectName %>));
        }
        ...
        public static bool CanDeleteObject(ApplicationContext appContext)
        {
            return BusinessRules.HasPermission(appContext, Csla.Rules.AuthorizationActions.DeleteObject, typeof(<%= Info.ObjectName %>));
        }
        <%
    }
    %>
