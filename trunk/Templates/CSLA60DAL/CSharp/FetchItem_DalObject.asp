...
{
    %>
    private List<<%= Info.ItemType %>Dto> Fetch(DataRowCollection rows)
    {
        ...
        {       
            ...
            if (!dr.IsNull("<%= prop.ParameterName %>"))
                <%= FormatCamel(Info.ItemType) %>.<%= FormatProperty(prop) %> = <%= GetDataRowStatement(prop) %>;
                    <%
                    }
                    catch (Exception ex)
                    {
            ...
            %>
            FetchChildren(<%= FormatCamel(Info.ItemType) %>, dr);
            <%= FormatCamel(Info.ObjectName) %>.Add(<%= FormatCamel(Info.ItemType) %>);
        }
