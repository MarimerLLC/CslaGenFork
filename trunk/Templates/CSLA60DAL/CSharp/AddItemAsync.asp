...
<%
        ...
        {
            %>
            var portal = ApplicationContext.GetRequiredService<IChildDataPortal<<%= Info.ItemType %>>>();
            var item = <%= Info.ItemType %>.<%= newMethodNameAsync %><%= crit.CreateOptions.FactorySuffix %>(<%= factoryParamsAsync %>, portal);
            <%
        }
        else
        {
            %>
            var portal = ApplicationContext.GetRequiredService<IChildDataPortal<<%= Info.ItemType %>>>();
            var item = await portal.Create<%= crit.CreateOptions.RunLocal ? "Child" : "" %>Async(<%= factoryParamsAsync %>);
            <%
        }
        %>
            Add(item);
