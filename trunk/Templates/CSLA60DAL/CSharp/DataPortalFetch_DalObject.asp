...
foreach (Criteria c in Info.CriteriaObjects)
{
	...
	%>
				...
				<%= AddIndent(needsIndent)%>OnFetchPre(args);
	<%
    if (usesDTO)
    {
        %>
                <%= AddIndent(needsIndent)%>var dr = cmd.ExecuteReader();
                <%= AddIndent(needsIndent)%>var data = <%= isCollection ? "LoadCollection(dr)" : "Fetch(dr)" %>;
                    <%
    }
    else
    {
        %>
                <%= AddIndent(needsIndent)%>var data =  cmd.ExecuteReader();
                    <%
    }
    %>
				<%= AddIndent(needsIndent)%>args = new DalHookArgs(cmd, <%= hookArgs %>)<%= usesDTO ? " { DtoArg = data }" : "" %>;
				<%= AddIndent(needsIndent)%>OnFetchPost(args);
				<%= AddIndent(needsIndent)%>return data;
            <%= AddIndent(needsIndent)%>}
            }
        }
        <%
