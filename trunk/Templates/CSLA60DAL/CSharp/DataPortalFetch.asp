...
if (!Info.UseCustomLoading && !Info.DataSetLoadingScheme)
{
    ...
    <%
        ...
        if (c.Properties.Count > 1)
        {
            if (c.CriteriaClassMode != CriteriaMode.MultiplePrimatives)
            {
    %>protected void Fetch(<%= c.Name %> crit, [Inject] I<%= Info.ObjectName %>Dal dal<%= Info.GetCollectionChildProperties().Count > 0 ? ", [Inject] IChildDataPortalFactory childFactory" : "" %>)<%
            }
            else
            {
    %>protected void Fetch(<%= ReceiveMultipleCriteria(c) %>, [Inject] I<%= Info.ObjectName %>Dal dal<%= Info.GetCollectionChildProperties().Count > 0 ? ", [Inject] IChildDataPortalFactory childFactory" : "" %>)<%
            }
        }
        else if (c.Properties.Count > 0)
        {
    %>protected void Fetch(<%= ReceiveSingleCriteria(c, "crit") %>, [Inject] I<%= Info.ObjectName %>Dal dal<%= Info.GetCollectionChildProperties().Count > 0 ? ", [Inject] IChildDataPortalFactory childFactory" : "" %>)<%
        }
        else
        {
    %>protected void Fetch([Inject] I<%= Info.ObjectName %>Dal dal<%= Info.GetCollectionChildProperties().Count > 0 ? ", [Inject] IChildDataPortalFactory childFactory" : "" %>)<%
        }
        %>
        {
            <%
            ...
            Fetch(data<%= (Info.GetCollectionChildProperties().Count > 0 && ancestorLoaderLevel > 0) ? ", childFactory" : "" %>);
                <%
            if (ParentLoadsChildren(Info) && usesDTO)
            {
                %>
            FetchChildren(dal, childFactory);
            <%
            }
                %>
            OnFetchPost(args);
    ...
    if (Info.HasGetCriteria && !usesDTO)
    {
        ...
        private void Fetch(IDataReader data<%= Info.GetCollectionChildProperties().Count > 0 ? ", [Inject] IChildDataPortalFactory childFactory" : "" %>)
        {
            using (var dr = new SafeDataReader(data))
            {
                if (dr.Read())
                {
                    Fetch(dr<%= (Info.GetCollectionChildProperties().Count > 0 && ancestorLoaderLevel > 0) ? ", childFactory" : "" %>);
                    <%
        if (ParentLoadsChildren(Info))
        {
            %>
                    FetchChildren(dr, childFactory);
                    <%
        }
        %>
                }
