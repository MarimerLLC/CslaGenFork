...
foreach (Criteria c in Info.CriteriaObjects)
{
    if (c.GetOptions.DataPortal)
    {
        ...
        List<<%= Info.ItemType %>Dto> Fetch<%= c.GetOptions.FactorySuffix %>(<%= ReceiveMultipleCriteria(c) %>);
        <%
            }
            else if (c.Properties.Count > 0)
            {
                %>
        /// <returns>A list of <see cref="<%= Info.ItemType %>Dto"/>.</returns>
        List<<%= Info.ItemType %>Dto> Fetch(<%= c.GetOptions.FactorySuffix %><%= ReceiveSingleCriteria(c, "crit") %>);
        <%
            }
            else
            {
                %>
        /// <returns>A list of <see cref="<%= Info.ItemType %>Dto"/>.</returns>
        List<<%= Info.ItemType %>Dto> Fetch<%= c.GetOptions.FactorySuffix %>();
        <%
            }
        }
        else
        ...
        IDataReader Fetch<%= c.GetOptions.FactorySuffix %>(<%= strGetCritParams %>);
        <%
        }
    }
}
