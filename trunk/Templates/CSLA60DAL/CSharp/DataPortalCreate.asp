...
foreach (Criteria c in Info.CriteriaObjects)
{
    if (c.CreateOptions.DataPortal)
    {
        ...
        if (c.Properties.Count > 1)
        {
            ...
            {
            %>
        protected void Create(<%= c.Name %> crit<%= Info.GetCollectionChildProperties().Count > 0 ? ", [Inject] IChildDataPortalFactory childFactory" : "" %>)<%
            }
            else
            {
            %>
        protected void Create(<%= ReceiveMultipleCriteria(c) %><%= Info.GetCollectionChildProperties().Count > 0 ? ", [Inject] IChildDataPortalFactory childFactory" : "" %>)<%
            }
        }
        else if (c.Properties.Count > 0)
        {
            %>
        protected void Create(<%= ReceiveSingleCriteria(c, "crit") %><%= Info.GetCollectionChildProperties().Count > 0 ? ", [Inject] IChildDataPortalFactory childFactory" : "" %>)<%
        }
        else
        {
            %>
        protected void Create(<%= Info.GetCollectionChildProperties().Count > 0 ? "[Inject] IChildDataPortalFactory childFactory" : "" %>)<%
        }
        %>
        {
            <%
